...
Chromecast.prototype.control = function(directive, videoId){
  if(this.channel && this.player){
    ...
    switch(directive){
      ...
      case 'play':
        this.player.control({ action : 'play', videoId : videoId });
        break;
    }
  }
};
    ...
    if(err){
      ...
      self.player.on('status', function(status){
        ...
        if(status.playerState == 'IDLE' && status.idleReason == 'FINISHED'){
          ...
          self.control('play', self.channel.videos[self.videoIndex].id);
        }
      });
    }
  });
